...
--5. 학번 A413042 인 박건우 학생의 주소가 "서울시 종로구 숭인동 181-21 "로 변경되었다고 핚다. 주소지를 정정하기 위해 사용핛 SQL 문을 작성하시오.
select *
from tb_student;
...
--7. 의학과 김명훈 학생은 2005 년 1 학기에 자신이 수강핚 '피부생리학' 점수가 잘못되었다는 것을 발견하고는 정정을 요청하였다.
--담당 교수의 확인 받은 결과 해당 과목의 학점을 3.5 로 변경키로 결정되었다. 적젃핚 SQL 문을 작성하시오.
...
no varchar2(5),
name varchar2(10),
CONSTRAINT PK_TB_CL_TY PRIMARY KEY(no)
);
...
add constraint pk_tb_alter_no primary key(NAME);
...
alter table tb_class_type
rename constraint SYS_C007208 to pk_class_type_no;
...
select student_name 학생이름,
        (select department_name from tb_department d where d.department_no= s.department_no) 학과이름,
        nvl((select professor_name from tb_professor p where p.professor_no = s.coach_professor_no),'교수없음') 교수명
from tb_student s
order by 학과이름;
...
select *
        from (select class_name,
                count(*)
        from tb_class c join tb_grade d
        using(class_no)
        where substr(d.term_no,1,4) in ('2009','2008','2007')
        group by class_name
        order by 2 desc
                )
where rownum <= 3;
